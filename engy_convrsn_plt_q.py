    ...
    if variable['operation'] == 'max':
            ...
            # Add text label for minimum value with adjusted position
             if variable['label'] == 'ΦEB':
              plt.text(date_values[min_idx], min_val, f'Min {variable["label"]}: {min_val:.2f}', fontsize=15, ha='right', va='center', color='black')
             else:
...
# Set the minor locator to AutoMinorLocator for 8 minor ticks between majors
plt.gca().xaxis.set_minor_locator(AutoMinorLocator(8))
ax1.tick_params(axis='x', which='major', width=2, length=4,labelsize=17,rotation=15)
...
ax1.tick_params(axis='y', labelsize=17)  # Adjust the labelsize as needed for the y-axis ticks
...
# Adjust Margins
plt.subplots_adjust(left=0.13, right=0.9, top=.95, bottom=0.18)
